...
def _pip_install_torch_gpu_aware() -> bool:
    """
    نصب torch با اولویتِ ساخت CUDA (GPU) اگه روی سیستم GPU انویدیا پیدا بشه؛
    فقط وقتی GPUای نیست میره سراغ نسخه‌ی سبک‌تر CPU-only.
    """
    has_gpu = _nvidia_gpu_present()

    if not has_gpu:
        print("[*] GPU انویدیا پیدا نشد → نصب torch (CPU) ...")
        return _pip_install("torch", "--index-url", "https://download.pytorch.org/whl/cpu")

    print("[*] GPU انویدیا پیدا شد → نصب torch با پشتیبانی CUDA ...")
    
    if _pip_install("torch"):
        return True

    print("[*] نصب پیش‌فرض torch ناموفق بود → تلاش با ایندکس‌های مشخصِ CUDA ...")
    for cu_tag in ("cu124", "cu121", "cu118"):
        print(f"    امتحان torch ({cu_tag}) ...")
        if _pip_install("torch", "--index-url", f"https://download.pytorch.org/whl/{cu_tag}"):
            return True

    print("[!] هیچ‌کدام از نسخه‌های GPU torch نصب نشدند → آخرین تلاش با CPU-only ...")
    return _pip_install("torch", "--index-url", "https://download.pytorch.org/whl/cpu")


def _pip_install_simple_lama() -> bool:
    """
    نصب simple-lama-inpainting با چند لایه fallback:
    ۱) نصب معمولی (اگه wheel آماده باشه، سریع و بی‌دردسر).
    ۲) اگه بیلد از سورس با خطای egg_info/metadata-generation-failed شکست خورد
       (رایج روی Python 3.13 به‌خاطر ناسازگاری setuptools/distutils قدیمی)،
       اول pip/setuptools/wheel رو آپدیت می‌کنیم و با --no-build-isolation
       دوباره امتحان می‌کنیم (بیلد از همون setuptools نصب‌شده‌ی محیط استفاده
       می‌کنه، نه یه محیط ایزوله‌ی جدا با نسخه‌ی قدیمی).
    """
    if _pip_install("simple-lama-inpainting"):
        return True

    print("    [*] نصب معمولی شکست خورد → آپدیت pip/setuptools/wheel ...")
    _pip_install("--upgrade", "pip", "setuptools", "wheel")

    print("    [*] تلاش دوباره با --no-build-isolation ...")
    cmd = [
        sys.executable, "-m", "pip", "install", "-q", "--prefer-binary",
        "--no-build-isolation", "simple-lama-inpainting",
    ]
    try:
        r = subprocess.run(cmd, capture_output=True, text=True, timeout=900)
        if r.returncode == 0:
            return True
        err = (r.stderr or r.stdout or "")[-800:]
        print(f"    [!] --no-build-isolation هم ناموفق بود: {err}")
    except Exception as e:
        print(f"    [!] خطا: {e}")
...
def _ensure_all_dependencies() -> None:
    ...
    if not _can_import("torch"):
        print("[*] نصب torch (لازم برای SimpleLama inpainting) ...")
        _pip_install_torch_gpu_aware()
        import importlib
        importlib.invalidate_caches()
    if not _can_import("simple_lama_inpainting"):
        print("[*] نصب simple-lama-inpainting ...")
        _pip_install_simple_lama()
        import importlib
        importlib.invalidate_caches()

    
    if not _can_import("torch"):
        print("[!] torch هنوز نصب/قابل‌بارگذاری نیست → SimpleLama غیرفعال می‌ماند (فقط OpenCV).", file=sys.stderr)
    elif not _can_import("simple_lama_inpainting"):
        try:
            __import__("simple_lama_inpainting")
        except Exception as _e:
            print(
                f"[!] simple-lama-inpainting بعد از نصب هم لود نشد ({type(_e).__name__}: {_e}) "
                f"→ SimpleLama غیرفعال می‌ماند (فقط OpenCV).",
                file=sys.stderr,
            )
...
except Exception as _lama_import_err:
    SimpleLama = None
    print(
        f"[!] simple-lama-inpainting در دسترس نیست "
        f"({type(_lama_import_err).__name__}: {_lama_import_err}) — پاک‌سازی فقط با OpenCV.",
        file=sys.stderr,
    )
...
class SimpleLamaInpainter:
    """
    اینپینتر جدید مبتنی بر simple-lama-inpainting (مدل واقعی big-lama، PyTorch)
    ...
    def __init__(self, mask_expand_ratio: Optional[float] = None):
        if SimpleLama is None:
            raise RuntimeError(
                "simple-lama-inpainting نصب نیست. نصب: pip install simple-lama-inpainting"
            )
        ...
        print("    [*] بارگذاری مدل SimpleLama (big-lama.pt) ...")
        self._model = SimpleLama()
        print("    [+] SimpleLama آماده است.")
    ...
    def _ensure_local_model_cached() -> None:
        ...
        try:
            ...
            print(f"    [!] کپیِ مدل محلیِ big-lama.pt ناموفق بود ({e})؛ به دانلود خودکار SimpleLama تکیه می‌کنیم.")
...
class MangaTranslator:
    ...
    def _decide_lama(self, force_gpu: Optional[bool]) -> bool:
        ...
        if not has_lama:
            print("[*] simple-lama-inpainting نیست → OpenCV inpaint.")
            return False

        if force_gpu is True:
            print(f"[*] --gpu → SimpleLama فعال ({name or ('CUDA' if has_cuda else 'CPU')}, {vram:.1f} GB).")
            return True

        if has_cuda and (vram <= 0 or vram >= self._LAMA_MIN_VRAM_GB):
            print(f"[*] GPU مناسب ({name or 'CUDA'}, {vram:.1f} GB) → SimpleLama (GPU).")
            return True
        ...
        print("[*] GPU نیست → OpenCV سریع. برای SimpleLama روی CPU: --lama")
        return False
