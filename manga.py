...
class MangaTranslator:
    ...
    def _build_text_mask(self, image: np.ndarray, regions: List[TextRegion]) -> np.ndarray:
        ...
        for region in regions:
            ...
            x1m, y1m = min(w_img, x1 + m), min(h_img, y1 + m)
            if x1m <= x0m or y1m <= y0m:
                continue

            poly_crop = poly_mask[y0m:y1m, x0m:x1m]
            poly_area = int(np.count_nonzero(poly_crop))
            if poly_area <= 0:
                continue
            ...
            try:
                ink_crop = self._ink_mask_inside_bubble(gray_full, x0m, y0m, x1m, y1m)
            except Exception:
                ink_crop = None

            if ink_crop is None or ink_crop.shape != poly_crop.shape:
                continue

            
            ink_crop = cv2.bitwise_and(ink_crop, poly_crop)
            ink_area = int(np.count_nonzero(ink_crop))
            if ink_area <= 0:
                
                continue

            poly_area_total += poly_area
            ink_area_total += ink_area

            region_mask = np.zeros((h_img, w_img), dtype=np.uint8)
            region_mask[y0m:y1m, x0m:x1m] = ink_crop
        ...
        raw_ink_mask = text_mask.copy()
        raw_ink_area = int(np.count_nonzero(raw_ink_mask))
        self._log_mask_area("ماسک اولیه (ink واقعی داخل OCR polygon)", raw_ink_area, h_img * w_img)
        if poly_area_total > 0:
            print(f"      نسبت ink/polygon-OCR: {100.0 * ink_area_total / poly_area_total:.1f}%")

        
        
        refined = self._refine_text_mask_tight(image, text_mask)
        refined_area = int(np.count_nonzero(refined))
        if refined_area > 0 and raw_ink_area > 0 and refined_area > 1.6 * raw_ink_area:
            print(f"      [!] CRF refine ماسک را بیش‌ازحد بزرگ کرد "
                  f"({refined_area} > 1.6×{raw_ink_area}) → نادیده گرفته شد.")
        else:
            text_mask = refined
        self._log_mask_area("ماسک بعد از refine", int(np.count_nonzero(text_mask)), h_img * w_img)
        ...
        if np.any(promo_sfx_mask):
            ...
            promo_sfx_dilated = cv2.dilate(promo_sfx_mask, small_kernel, iterations=1)
            text_mask = cv2.bitwise_or(text_mask, promo_sfx_dilated)

        
        
        pad = max(0, min(2, int(getattr(self, "mask_padding", 3) or 0)))
        if pad > 0:
            k = 2 * pad + 1
            pad_kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (k, k))
            text_mask = cv2.dilate(text_mask, pad_kernel, iterations=1)

        final_area = int(np.count_nonzero(text_mask))
        self._log_mask_area("ماسک نهایی (بعد از padding)", final_area, h_img * w_img)

        
        
        
        if raw_ink_area > 0 and final_area > 3.0 * raw_ink_area:
            print(f"      [!] ماسک نهایی نسبت به ink واقعی خیلی بزرگ شده "
                  f"({final_area} > 3×{raw_ink_area}) → محدود به ماسک ink اولیه + کمی padding شد.")
            safe_kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3))
            text_mask = cv2.dilate(raw_ink_mask, safe_kernel, iterations=1)
            self._log_mask_area("ماسک نهایی (محدودشده)", int(np.count_nonzero(text_mask)), h_img * w_img)

        return text_mask

    @staticmethod
    def _log_mask_area(label: str, area_px: int, total_px: int) -> None:
        
        try:
            ratio = 100.0 * float(area_px) / float(max(1, total_px))
            print(f"  [mask-log] {label}: {area_px}px ({ratio:.3f}% از تصویر)")
        except Exception:
            pass
    ...
    def _refine_text_mask_tight(self, image: np.ndarray, mask: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            refined_fg = int(np.count_nonzero(refined))

            
            
            if refined_fg < 0.05 * fg:
                return mask
            
            if refined_fg > 1.6 * fg:
                return mask
            return refined
        except Exception:
            return mask
    ...
    def _flat_fill_uniform_components(
        ...
        for i in range(1, n):
            ...
            cy = 0.5 * (ys_ring.min() + ys_ring.max())
            cx = 0.5 * (xs_ring.min() + xs_ring.max())
            quads = [
                (ys_ring < cy) & (xs_ring < cx),
                (ys_ring < cy) & (xs_ring >= cx),
                (ys_ring >= cy) & (xs_ring < cx),
                (ys_ring >= cy) & (xs_ring >= cx),
            ]
            quad_means = []
            for q in quads:
                if int(np.count_nonzero(q)) < 4:
                    continue
                quad_means.append(image[ys_ring[q], xs_ring[q]].astype(np.float32).mean(axis=0))
            is_gradient = False
            if len(quad_means) >= 2:
                qarr = np.stack(quad_means, axis=0)
                spread = float(np.max(qarr, axis=0).max() - np.min(qarr, axis=0).min())
                if spread > gradient_thresh:
                    is_gradient = True
    ...
    def clean_image(self, image: np.ndarray, regions: List[TextRegion]) -> np.ndarray:
            ...
            for region in regions:
                ...
                for poly in polys:
                    try:
                        ...
                        total_poly_px += max(0, int(x1) - int(x0)) * max(0, int(y1) - int(y0))
                    except Exception:
                        continue
            mask_before_px = total_poly_px
        except Exception:
            pass
        ...
        mask_after_px = int((mask > 0).sum())
        try:
            ratio = float(mask_after_px) / float(mask.size)
            print(f"  [*] ماسک متن نهایی: {ratio*100:.2f}% از کل تصویر")
            if mask_before_px > 0:
                growth = mask_after_px / float(mask_before_px)
                print(f"  [*] رشد ماسک نسبت به bbox خام OCR: {growth:.2f}x "
                      f"({mask_before_px}px → {mask_after_px}px)")
        except Exception:
            pass
        ...
        if getattr(self, "use_lama", False):
            ...
            if lama is not None:
                try:
                    ...
                    cleaned[m] = out[m]
                    lama_done = True
                    print("  - پاکسازی باقی‌مونده‌ها با LaMa انجام شد.")
                except Exception as e:
                    print(f"  [!] LaMa خطا ({e}) → OpenCV inpaint.")
    ...
    def _opencv_inpaint_two_pass(self, image: np.ndarray, mask: np.ndarray) -> np.ndarray:
        ...
        before_px = int(np.count_nonzero(m))
        after_px = int(np.count_nonzero(m_dilated))
        if before_px > 0 and after_px > 2.0 * before_px:
            
            m_dilated = m

        
        r1 = max(3, min(5, int(getattr(self, "inpaint_radius", 3) or 3)))
    ...
    def _get_system_instruction(self) -> str:
        return (
            """تو یه مترجم حرفه‌ای دیالوگ مانگا/مانهواست که متن رو به فارسیِ کاملاً طبیعی و محاوره‌ای
برمی‌گردونه؛ طوری که خواننده حس کنه شخصیت واقعاً داره فارسی حرف می‌زنه، نه اینکه متن از
یه زبان دیگه ترجمه شده. تو داری یه فصل کامل رو (نه یه صفحه‌ی تنها) توی چند Batch
پشت‌سرهم ترجمه می‌کنی، پس باید دقیقاً مثل یه مترجم واقعی که کل فصل رو خونده و فهمیده،
پیوستگی داستان، لحن شخصیت‌ها و اصطلاح‌ها رو بین Batchها حفظ کنی؛ صفحه‌ها یا Batchها رو
مستقل و ایزوله از هم ترجمه نکن.
...
- هرگز کلمه‌به‌کلمه ترجمه نکن؛ مفهوم و حس جمله رو با ساختار طبیعی فارسی بازنویسی کن.

💬 دیالوگ متناسب با شخصیت:
...
(با توجه به Context کل فصل، لحن هر شخصیت رو حفظ و متمایز کن)

🗣️ قوانین محاوره‌ای‌سازی:
«می‌خوام» نه «می‌خواهم» | «چیکار» نه «چه کار» | «بذار» نه «بگذار» | «میشه» نه «می‌شود» |
«نمیشه» نه «نمی‌شود» | «واسه» نه «برای» | «رو» نه «را» | «اگه» نه «اگر» | «آخه» نه «آخر» |
«اون» نه «آن» | «این‌جوری» نه «این‌گونه» | «ارزون» نه «ارزان»
فعل‌ها شکسته: می‌کنه، می‌گه، نمی‌دونم، اومد، رفت، دارم، می‌تونم، بهش، شون (نه شان).
ممنوع: می‌باشد، است (به‌جای «ه»)، را، خواهید، ایشان، این‌گونه، متأسفانه، لذا، بدین‌ترتیب،
در نتیجه.

✍️ روان بودن متن:
- جمله‌ها کوتاه و جمع‌وجور؛ از جمله‌ی طولانی و پیچیده پرهیز کن.
- کلمات قلمبه‌سلمبه فقط در صورت نیاز.
- تا جای ممکن با کمترین کلمه همون مفهوم رو منتقل کن (چون باید توی بالن جا بشه).

فحش‌ها رو طبیعی و کامل ترجمه کن، سانسور نکن:
What the fuck are you doing?! → چه گوهی داری می‌خوری؟! | fuck you → گاییدمت |
shit → کیری / گوه
...
روایت و لحن شخصیت‌ها رو در کل فصل حفظ کن، به فارسیِ خودمونی و روون ترجمه کن؛ کوتاه، طبیعی، بدون کلمه‌ی کتابی. فحش‌ها رو
کامل و طبیعی بیار. املای همه‌ی کلمات باید صد در صد درست باشه. برای اسم‌ها و اصطلاح‌های
