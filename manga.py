    ...
    def _build_text_mask(self, image: np.ndarray, regions: List[TextRegion]) -> np.ndarray:
        ...
        for region in regions:
            ...
            try:
                ...
                bx0 = max(0, int(rx))
                by0 = max(0, int(ry))
                bx1 = min(w_img, int(rx + rw))
                by1 = min(h_img, int(ry + rh))
                if bx1 - bx0 >= 8 and by1 - by0 >= 8:
                    ink = self._ink_mask_inside_bubble(gray_full, bx0, by0, bx1, by1)
                    gray_crop = gray_full[by0:by1, bx0:bx1]
                    ink = self._protect_bubble_wall(ink, gray_crop)
                    ch_i, cw_i = ink.shape[:2]
                    ink = self._drop_non_text_components(ink, ch_i, cw_i)
                    zone = self._text_zone_in_crop(region, bx0, by0, bx1, by1)
                    if zone is not None:
                        near = cv2.dilate(zone, np.ones((5, 5), np.uint8), iterations=4)
                        ink = cv2.bitwise_and(ink, near)
                    if np.any(ink):
                        region_mask[by0:by1, bx0:bx1] = cv2.bitwise_or(
                            region_mask[by0:by1, bx0:bx1], ink
                        )
            except Exception:
                pass
