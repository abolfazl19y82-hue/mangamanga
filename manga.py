...
# ══════════════════════════════════════════════════════════════════════
# Persian Text Polish — مرحله‌ی محلی، بدون API/اینترنت، بعد از ترجمه
# فقط غلط‌های واضح املایی/نیم‌فاصله/نگارشی را اصلاح می‌کند؛ لحن، معنی،
# اسم‌ها و اصطلاحات دست‌نخورده می‌مانند.
# ══════════════════════════════════════════════════════════════════════

_PERSIAN_POLISH_DICT: Dict[str, str] = {
    
    "مسول": "مسئول", "مسولین": "مسئولین",
    "مسوليت": "مسئولیت", "مسولیت": "مسئولیت",
    "مساله": "مسئله", "مسايل": "مسائل",
    
    "میخوام": "می‌خوام", "می خوام": "می‌خوام",
    "میخواستم": "می‌خواستم", "می خواستم": "می‌خواستم",
    "میخواد": "می‌خواد", "می خواد": "می‌خواد",
    "میدونم": "می‌دونم", "می دونم": "می‌دونم",
    "نمیدونم": "نمی‌دونم", "نمی دونم": "نمی‌دونم",
    "میدونی": "می‌دونی", "می دونی": "می‌دونی",
    "نمیدونی": "نمی‌دونی", "نمی دونی": "نمی‌دونی",
    "میدونه": "می‌دونه", "می دونه": "می‌دونه",
    "نمیدونه": "نمی‌دونه", "نمی دونه": "نمی‌دونه",
    "میتونم": "می‌تونم", "می تونم": "می‌تونم",
    "نمیتونم": "نمی‌تونم", "نمی تونم": "نمی‌تونم",
    "میتونی": "می‌تونی", "می تونی": "می‌تونی",
    "نمیتونی": "نمی‌تونی", "نمی تونی": "نمی‌تونی",
    "میتونه": "می‌تونه", "می تونه": "می‌تونه",
    "نمیتونه": "نمی‌تونه", "نمی تونه": "نمی‌تونه",
    "میکنم": "می‌کنم", "می کنم": "می‌کنم",
    "نمیکنم": "نمی‌کنم", "نمی کنم": "نمی‌کنم",
    "میکنه": "می‌کنه", "می کنه": "می‌کنه",
    "نمیکنه": "نمی‌کنه", "نمی کنه": "نمی‌کنه",
    "میکنی": "می‌کنی", "می کنی": "می‌کنی",
    "نمیکنی": "نمی‌کنی", "نمی کنی": "نمی‌کنی",
    "میگم": "می‌گم", "می گم": "می‌گم",
    "نمیگم": "نمی‌گم", "نمی گم": "نمی‌گم",
    "میگه": "می‌گه", "می گه": "می‌گه",
    "نمیگه": "نمی‌گه", "نمی گه": "نمی‌گه",
    "میره": "می‌ره", "می ره": "می‌ره",
    "نمیره": "نمی‌ره", "نمی ره": "نمی‌ره",
    "میاد": "می‌آد", "می اد": "می‌آد",
    "نمیاد": "نمی‌آد", "نمی اد": "نمی‌آد",
    ...
    "آن ها": "آن‌ها", "این ها": "این‌ها",
}

_PERSIAN_HALF_SPACE_SUFFIXES = ("ایم", "اید", "اند", "ام", "ای")

_PERSIAN_PROTECT_RE = re.compile(
    r"(https?://\S+|www\.\S+"
    r"|\{[^{}]*\}"
    r"|\[[^\[\]]*\]"
    r"|<[^<>]*>"
...
def polish_persian_text(text: str) -> str:
    ...
    guarded = _PERSIAN_PROTECT_RE.sub(_protect, text)

    suf_pat = "|".join(_PERSIAN_HALF_SPACE_SUFFIXES)
    ...
    for line in guarded.split("\n"):
        ...
        for wrong, right in _PERSIAN_POLISH_DICT.items():
            if wrong in fl:
                fl = re.sub(
                    rf"(?<![\u0600-\u06FF]){re.escape(wrong)}(?![\u0600-\u06FF])",
                    right, fl,
                )
        ...
        fl = re.sub(r"([،؛؟!])(?=[\u0600-\u06FF])", r"\1 ", fl)
        
        fl = re.sub(r"[ \t]{2,}", " ", fl)

        fixed_lines.append(fl.strip(" \t"))

    result = "\n".join(fixed_lines)
    for i, val in enumerate(protected):
        result = result.replace(f"\x00{i}\x00", val)
    return result
...
class MangaTranslator:
    ...
    def __init__(
        ...
        persian_polish: bool = PERSIAN_POLISH_ENABLED,
    ):
        ...
        self.persian_polish_enabled = bool(persian_polish)
        ...
        for _src, _per in self._name_glossary.items():
            self.translation_memory.setdefault(
                _src, {"translated_term": _per, "type": "", "character": "", "notes": ""}
            )
        self._lama = None
...
"terms": [{"original_term": "...", "translated_term": "...", "type": "...",
"character": "...", "notes": "..."}]}
tone (الزامی) یکی از:
...
terms فقط برای اسم‌خاص، اصطلاح تخصصی یا عبارت مهمی بفرست که تازه معرفی شده یا از
حافظه‌ی ترجمه استفاده شده و باید ثبت/تثبیت بشه؛ در غیر این صورت آرایه رو خالی بذار."""
    ...
    def _parse_translation_response(self, text: str, regions: List[TextRegion]) -> bool:
        ...
        for region in regions:
            ...
            if t:
                ...
                if self.persian_polish_enabled:
                    t = polish_persian_text(t)
        ...
        for item in results:
            for nm in (item.get("names") or []):
                ...
                if src and per and src not in self._name_glossary:
                    ...
                    self.translation_memory.setdefault(
                        src, {"translated_term": per, "type": "", "character": "", "notes": ""}
                    )
                    self._tm_dirty = True
            
            for tm in (item.get("terms") or []):
                if not isinstance(tm, dict):
                    continue
                src = str(tm.get("original_term") or "").strip()
                per = str(tm.get("translated_term") or "").strip()
                if not (src and per):
                    continue
                typ = str(tm.get("type") or "").strip()
                if typ not in self.TM_TYPES:
                    typ = ""
                ch = str(tm.get("character") or "").strip()
                notes = str(tm.get("notes") or "").strip()
                existing = self.translation_memory.get(src)
                if existing != {"translated_term": per, "type": typ, "character": ch, "notes": notes}:
                    self.translation_memory[src] = {
                        "translated_term": per, "type": typ, "character": ch, "notes": notes,
                    }
                    self._tm_dirty = True
                if src not in self._name_glossary:
                    self._name_glossary[src] = per
                    self._glossary_dirty = True
        return applied > 0
                        ...
                        "terms": {
                            ...
                            "items": {
                                ...
                                "properties": {
                                    ...
                                    "type": {
                                        "type": "STRING",
                                        "enum": list(self.TM_TYPES) + [""],
                                    },
                                    "character": {"type": "STRING"},
                                    "notes": {"type": "STRING"},
                                },
                                "required": ["original_term", "translated_term"],
                            },
                        },
                    },
    ...
    TM_TYPES = (
        "character", "location", "organization", "technique", "item",
        "title", "nickname", "terminology", "recurring_phrase",
    )
    ...
    def set_glossary_output_dir(self, out_dir: str) -> None:
            ...
            try:
                ...
                for k, v in d.items():
                    ...
                    if k and v and k not in self._name_glossary:
                        ...
                        self.translation_memory.setdefault(
                            k, {"translated_term": v, "type": "", "character": "", "notes": ""}
                        )
                if d:
        ...
        auto_tm = os.path.join(self._glossary_out_dir, "translation_memory.json")
        if os.path.isfile(auto_tm):
            try:
                import json as _json
                with open(auto_tm, encoding="utf-8") as f:
                    d = _json.load(f)
                added = 0
                for k, info in (d or {}).items():
                    k = str(k).strip()
                    if not k or k in self.translation_memory or not isinstance(info, dict):
                        continue
                    tr = str(info.get("translated_term", "")).strip()
                    if not tr:
                        continue
                    self.translation_memory[k] = {
                        "translated_term": tr,
                        "type": str(info.get("type", "")).strip(),
                        "character": str(info.get("character", "")).strip(),
                        "notes": str(info.get("notes", "")).strip(),
                    }
                    self._name_glossary.setdefault(k, tr)
                    added += 1
                if added:
                    print(f"[*] translation_memory.json قبلی بارگذاری شد: {added} مورد")
            except Exception:
                pass
    ...
    def save_translation_memory(self) -> None:
        if not self._tm_dirty or not self.translation_memory:
            return
        out_dir = self._glossary_out_dir or os.getcwd()
        try:
            import json as _json
            os.makedirs(out_dir, exist_ok=True)
            path = os.path.join(out_dir, "translation_memory.json")
            with open(path, "w", encoding="utf-8") as f:
                _json.dump(self.translation_memory, f, ensure_ascii=False, indent=2)
            print(f"[*] حافظه‌ی ترجمه ذخیره شد: {path} ({len(self.translation_memory)} مورد)")
            self._tm_dirty = False
        except Exception as e:
            print(f"[!] ذخیرهٔ حافظه‌ی ترجمه ناموفق ({e})")

    def _tm_prompt_block(self) -> str:
        if not self.translation_memory:
            return ""
        lines = []
        for term, info in sorted(self.translation_memory.items()):
            tr = (info.get("translated_term") or "").strip()
            if not tr:
                continue
            extra = []
            typ = (info.get("type") or "").strip()
            ch = (info.get("character") or "").strip()
            if typ:
                extra.append(typ)
            if ch:
                extra.append(f"شخصیت: {ch}")
            suffix = f"  [{', '.join(extra)}]" if extra else ""
            lines.append(f"  {term} → {tr}{suffix}")
        return "\n".join(lines)

    def _glossary_prompt_block(self) -> str:
        
        tm_lines = self._tm_prompt_block()
        body = (
            "ورودی‌های فعلیِ حافظه‌ی ترجمه (ازش استفاده کن، تغییرش نده):\n"
            f"{tm_lines}\n"
            if tm_lines else
            "فعلاً حافظه‌ی ترجمه خالیه؛ هر اسم خاص یا اصطلاح مهمی که توی این "
            "Batch می‌بینی رو برای Batchها/صفحه‌های بعدی ثبت کن.\n"
        )
        return (
            "\n━━━━━━━━━━━━━━━━━━━━\n"
            "🗂 حافظه‌ی ترجمه (Translation Memory) — قانون سازگاری اجباری\n"
            "━━━━━━━━━━━━━━━━━━━━\n"
            "این پروژه یه حافظه‌ی ترجمه‌ی پایدار داره که اسم شخصیت‌ها، مکان‌ها، "
            "سازمان‌ها، تکنیک‌ها، آیتم‌ها، عنوان‌ها، لقب‌ها، اصطلاح‌های تخصصی و "
            "عبارت‌های تکرارشونده‌ی این پروژه رو نگه می‌داره. اگه یه اصطلاح یا اسم "
            "قبلاً توی این پروژه (یا Batchهای قبلیِ همین فصل) ترجمه شده، دقیقاً همون "
            "ترجمه‌ی قبلی رو دوباره استفاده کن و بدون دلیل مشخص عوضش نکن (مثلاً اگه "
            "یه بار \"Golden Palace\" شده «قصر طلایی»، توی صفحه‌های بعدی نباید بشه "
            "«کاخ طلایی»؛ همینطور اسم شخصیت‌ها باید توی کل فصل عیناً ثابت بمونن). "
            "فقط در صورتی که بافت جمله‌ی جدید نشون بده اون ترجمه‌ی قبلی واقعاً غلط "
            "بوده، می‌تونی تغییرش بدی، اون‌وقت هم باید دلیلش رو کوتاه توی notes "
            "بنویسی.\n"
            f"{body}\n"
            "برای هر اسم خاص، اصطلاح تخصصی، تکنیک، آیتم، لقب، عنوان یا عبارت "
            "تکرارشونده‌ی مهمی که توی این Batch دیدی (چه جدید، چه از حافظه‌ی بالا)، "
            "یه ورودی توی آرایه‌ی terms برگردون با فیلدهای:\n"
            "- original_term: متن اصلی (انگلیسی/زبان مبدأ)\n"
            "- translated_term: ترجمه‌ی فارسی که همین الان استفاده کردی\n"
            "- type: یکی از این مقادیر: " + ", ".join(self.TM_TYPES) + "\n"
            "- character: اگه این اصطلاح مال یا مربوط به یه شخصیت خاصه، اسمش رو "
            "بنویس (فارسی)؛ وگرنه خالی بذار\n"
            "- notes: توضیح کوتاه اختیاری (مثلاً چرا این ترجمه انتخاب شده، یا چرا "
            "نسبت به حافظه‌ی قبلی تغییر کرده)؛ وگرنه خالی بذار\n"
        )
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--no-persian-polish", action="store_true",
                   help="مرحله‌ی اصلاح محلی نگارشی/املایی/نیم‌فاصله‌ی متن فارسی بعد از "
                        "ترجمه غیرفعال شود (پیش‌فرض: فعال، بدون نیاز به API/اینترنت)")
...
def main():
    ...
    translator = MangaTranslator(
        ...
        persian_polish=not getattr(args, "no_persian_polish", False),
    )
