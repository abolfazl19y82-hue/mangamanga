...
class MangaTranslator:
    ...
    def _tighten_region_mask_to_ink(self, image: np.ndarray, region_mask: np.ndarray) -> np.ndarray:
        ...
        x1p, y1p = min(w_img, x1 + pad), min(h_img, y1 + pad)
        ch, cw = y1p - y0p, x1p - x0p
        if ch < 8 or cw < 8:
            return region_mask

        zone = region_mask[y0p:y1p, x0p:x1p]
        zone_cov = float(np.count_nonzero(zone)) / float(max(1, ch * cw))

        try:
            gray_crop = cv2.cvtColor(image[y0p:y1p, x0p:x1p], cv2.COLOR_BGR2GRAY)
            ink = self._letters_mask_in_crop(gray_crop, zone, ch, cw, wide=True)
        except Exception:
            return region_mask

        ink_cov = float(np.count_nonzero(ink)) / float(max(1, ch * cw))
        
        
        if ink_cov < 0.01 or ink_cov > 0.90 * max(zone_cov, 0.01):
            return region_mask

        tightened = np.zeros_like(region_mask)
        tightened[y0p:y1p, x0p:x1p] = ink
        tightened = cv2.bitwise_and(tightened, region_mask)
    ...
    def _build_text_mask(self, image: np.ndarray, regions: List[TextRegion]) -> np.ndarray:
        ...
        for region in regions:
            ...
            region_mask = self._tighten_region_mask_to_ink(image, region_mask)
    ...
    def clean_image(self, image: np.ndarray, regions: List[TextRegion]) -> np.ndarray:
        ...
        if not np.any(remaining):
            ...
            return self._harmonize_with_background(image, cleaned, mask)
    ...
    def _harmonize_with_background(
        ...
        for i in range(1, n):
            ...
            eroded = cv2.erode(comp_mask, np.ones((3, 3), np.uint8), iterations=1)
            ...
            cx = min(max(cx, x + 2), x + w - 3) if w > 4 else cx
            cy = min(max(cy, y + 2), y + h - 3) if h > 4 else cy
            cx = max(1, min(w_img - 2, cx))
            cy = max(1, min(h_img - 2, cy))

            try:
                blended = cv2.seamlessClone(
                    out, original, comp_mask, (cx, cy), cv2.NORMAL_CLONE
                )
                keep = comp_mask > 0
                out[keep] = blended[keep]
            except Exception:
