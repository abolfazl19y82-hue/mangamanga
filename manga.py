...
class MangaTranslator:
    ...
    def __init__(
        ...
        if use_paddle:
            ...
            ocr_kwargs = dict(
                lang=main_lang,
                show_log=False,
                # آستانه‌های تشخیص کمی حساس‌تر شدن (نسبت به ۰٫۲۵/۰٫۴/۱٫۸ قبلی)
                # تا لترینگِ خیلی استایلی/دستی (مثل افکت‌های صوتیِ کمیکی، حروفِ
                # کج‌ومعوجِ ضخیم) هم به‌عنوان region تشخیص داده بشه. تریدآف:
                # ممکنه گاهی یه خط‌خطیِ تصویری هم اشتباهی به‌عنوان متن گرفته
                # بشه (فیلترهای بعدیِ min_confidence/_classify_text تا حدی
                # این false-positiveها رو حذف می‌کنن).
                text_det_thresh=0.16,
        ...
        for region in regions:
            ...
            if zone is not None:
                ...
                if med_reg < 120:
                    ...
                    ink = zone
                    trusted_zone_fill = True
                else:
                    ink_in_zone = cv2.bitwise_and(ink, zone)
                    zone_area = float(np.count_nonzero(zone))
                    ink_area = float(np.count_nonzero(ink_in_zone))
                    
                    
                    
                    if zone_area > 0 and (ink_area / zone_area) < 0.35:
                        ink = zone
                        trusted_zone_fill = True
                    else:
                        ink = ink_in_zone
            ...
            if med_reg < 120 and ink.max() > 0:
                ...
                seed_area = float(np.count_nonzero(ink))
                grow = (ink > 0) & bright_m
                grow = grow | (ink > 0)
                
                
                
                grow_cap = min(0.70 * ch * cw, max(9.0 * seed_area, 0.12 * ch * cw))
                for _ in range(8):
                    add = cv2.dilate(grow.astype(np.uint8), np.ones((3, 3), np.uint8)).astype(bool) & bright_m & ~grow
                    if not add.any():
                        break
                    grow |= add
                    if float(np.count_nonzero(grow)) > grow_cap:
                        break
            ...
            if med_reg < 120:
                ...
                if 0.20 < cov_big <= 0.35:
                    ink = cv2.dilate(ink, np.ones((3, 3), np.uint8), iterations=3)

            
            
            
            
            if ink.max() > 0 and not trusted_zone_fill:
            ...
            if ink.max() > 0 and not trusted_zone_fill:
                cov = float(np.count_nonzero(ink)) / float(max(1, ch * cw))
                skip_thresh = 0.45 if zone is None else 0.55
                if cov > skip_thresh:
                    print(
                        f"    [!] ماسک ناحیه ({x0},{y0}) غیرقابل‌اعتماد بود "
                        f"({cov*100:.0f}٪) → پاک‌سازی این ناحیه رد شد "
                        f"(برای جلوگیری از پاک شدن نقاشی/بالن)."
                    )
                    continue
    ...
    def clean_image(self, image: np.ndarray, regions: List[TextRegion]) -> np.ndarray:
        ...
        for region in regions:
            ...
            if zone is not None:
                near = cv2.dilate(zone, np.ones((3, 3), np.uint8), iterations=12)
                
                near = cv2.bitwise_and(
                    near,
                    cv2.dilate(dil[y0:y1, x0:x1], np.ones((3, 3), np.uint8), iterations=10),
                )
            else:
    ...
    def _wrap_and_fit(
        ...
        _merged_words: List[str] = []
        for _w in words:
            if _merged_words and _punct_only.match(_w):
                _merged_words[-1] = _merged_words[-1] + _w
            else:
...
"""
    ...
    def _render_one_region(self, pil_img, draw, image, original_image, region) -> None:
        ...
        if short and (w < 140 or h < 70):
            
            
            expand = max(6, int(min(w, h) * 0.18))
            x = max(0, x - expand // 2)
            y = max(0, y - expand // 2)
            w = w + expand
            h = h + expand
