...
class LamaMangaONNX:
    ...
    def __call__(self, image, mask):
        ...
        hsv_probe = cv2.cvtColor(img_rgb, cv2.COLOR_RGB2HSV)
        is_grayscale_like = float(np.mean(hsv_probe[:, :, 1])) < 18.0
        invert = is_grayscale_like and float(np.median(cv2.cvtColor(img_rgb, cv2.COLOR_RGB2GRAY))) < 110
        if invert:
            img_rgb = 255 - img_rgb
...
class MangaTranslator:
    ...
    def _build_text_mask(self, image: np.ndarray, regions: List[TextRegion]) -> np.ndarray:
        ...
        try:
            n_comp, labels_comp = cv2.connectedComponents(
                (text_mask > 0).astype(np.uint8), connectivity=8
            )
            extra = np.zeros_like(text_mask)
            for i in range(1, n_comp):
                comp_mask = (labels_comp == i).astype(np.uint8) * 255
                bx, by, bw, bh = cv2.boundingRect(comp_mask)
                pad = 4
                x0, y0 = max(0, bx - pad), max(0, by - pad)
                x1, y1 = min(w_img, bx + bw + pad), min(h_img, by + bh + pad)
                comp_crop = comp_mask[y0:y1, x0:x1]
                if not np.any(comp_crop):
                    continue
                img_crop = image[y0:y1, x0:x1].astype(np.float32)
                ink_px = img_crop[comp_crop > 0]
                if ink_px.size == 0:
                    continue
                ink_color = np.median(ink_px, axis=0)

                
                ring_local = cv2.dilate(comp_crop, np.ones((7, 7), np.uint8), iterations=1)
                ring_local = cv2.bitwise_and(ring_local, cv2.bitwise_not(comp_crop))
                if not np.any(ring_local):
                    continue

                diff = np.linalg.norm(img_crop - ink_color, axis=2)
                close = (diff < 18.0) & (ring_local > 0)
                if np.any(close):
                    extra_crop = extra[y0:y1, x0:x1]
                    extra_crop[close] = 255
                    extra[y0:y1, x0:x1] = extra_crop

            if np.any(extra):
                text_mask = cv2.bitwise_or(text_mask, extra)
        except Exception:
            pass
