...
class MangaTranslator:
    ...
    def _build_text_mask(self, image: np.ndarray, regions: List[TextRegion]) -> np.ndarray:
        ...
        for region in regions:
            ...
            elif getattr(region, "kind", "") == "dialogue":
                
                
                rx, ry, rw, rh = region.rect
                mgx = max(10, int(rw * 0.6))
                mgy = max(10, int(rh * 0.6))
                cx0 = max(0, rx - mgx)
                cy0 = max(0, ry - mgy)
                cx1 = min(w_img, rx + rw + mgx)
                cy1 = min(h_img, ry + rh + mgy)
                if cx1 - cx0 >= 8 and cy1 - cy0 >= 8:
                    gray_crop = cv2.cvtColor(image[cy0:cy1, cx0:cx1], cv2.COLOR_BGR2GRAY)
                    zone = region_mask[cy0:cy1, cx0:cx1]
                    try:
                        interior = self._bubble_interior_mask(gray_crop, zone)
                    except Exception:
                        interior = None
                    if interior is not None:
                        text_mask[cy0:cy1, cx0:cx1] = cv2.bitwise_or(
                            text_mask[cy0:cy1, cx0:cx1], interior
                        )
...
🏷️ پسوند/سافیکسِ اسم‌های کره‌ای:
پسوندهایی که به آخر اسم یا نسبت می‌چسبن (مثل 오빠/오빠야، 언니، 형، 누나، 씨، 님، 선배،
아/야) رو اصلاً ترجمه نکن؛ همون‌طور که هستن (بدون تغییر، عین زبان اصلی) نگه دار و
دست‌نخورده رها کن. این قانون فقط مخصوص پسوند اسمه؛ باقی متن (از جمله تابلو،
نوشته‌های محیط، دیالوگ‌های عادی و بقیه‌ی جمله) رو طبق همون قوانین بالا کامل و طبیعی
ترجمه کن.
