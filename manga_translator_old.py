    ...
    def _refine_text_mask_tight(self, image: np.ndarray, raw_mask: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            if refined is None or refined.shape != raw_mask.shape:
                return raw_mask
            # ایمنیِ لبه: تنگ‌کردن گاهی حاشیه‌ی نازکِ دورِ حروف (سریف/آنتی‌الیاس)
            # رو بیرون می‌ندازه و باعث می‌شه بعد از پاکسازی، ذراتِ ریزِ متنِ
            # اصلی زیرِ متنِ ترجمه‌شده باقی بمونه. یه دیلیتِ خیلی کوچیک (۲px)
            # این حاشیه رو برمی‌گردونه بدون این‌که فایده‌ی اصلیِ تنگ‌کردن
            # (جلوگیری از پاکسازیِ بیش‌ازحدِ زمینه‌ی اطراف) از بین بره.
            safety_kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))
            refined = cv2.dilate(refined, safety_kernel, iterations=1)
            return refined
        except Exception as e:
            if mode == "crf":
                raise
